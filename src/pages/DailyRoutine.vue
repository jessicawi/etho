    ...
    <div id='dailyroutine'>
        <div class="container daily-header__wrap" style="background: transparent;">
            <div class="daily-header">
                <div class="title">
                    ...
                    <div class="daily-title_item">
                        <router-link :to="{name: 'Daily Routine Single Update'}">
        ...
        <div class="dailyRoutine-student container"
            ...
            <div class="row fullwidth">
                <div v-if="selectedClassStudent&&selectedClassStudent.length>0" class="responsive-table col-lg-12">
                    <table class="attTable fullwidth">
                        <tr>
                            <th>
                        ...
                        <tr v-for="(item,i) in selectedClassStudent" ref="studentList_Update"
                            ...
                            <td class="">
                                <div v-for="obj in item.rest" class="text-left">
                                    <span class="text-left">{{obj.DrReferenceType}} {{obj.DrStartTime_convert}} - {{obj.DrEndTime_convert}}</span>
                                </div>
                                <el-button @click="toggleInput(item)" class="link-button float-left"
                                           v-if="item.hideToggle !== true">
                                    <i class="material-icons">add</i>
                                    Add
                                </el-button>
                                <el-form>
                                    <el-form-item v-if="item.selectionRest !== false">
                                        <el-select ref="RestType" placeholder="Select" v-model="ddlRest"
                                                   @change="(e)=>AddAction(e,item,'step1')">
                                            <el-option value="Nap">Nap</el-option>
                                            <el-option value="Sleep">Sleep</el-option>
                                        </el-select>
                                    </el-form-item>
                                    <div v-if="item.selectionCurrentTime === true">
                                        <el-time-select
                                                v-model="txtTimeStart"
                                                :picker-options="{ start: '08:30', step: '00:15', end: '18:30' }"
                                                placeholder="Current Time"
                                                @change="(e)=>AddAction(e,item,'step2')">
                                        </el-time-select>
                                    </div>
                                    <el-form-item v-if="item.selectionEndTime === true">
                                        <el-time-select
                                        ...
                                        minTime: txtTimeStart
                                                ...
                                                @change="(e)=>AddAction(e,item,'final')">
                                        </el-time-select>
                                    </el-form-item>
                                </el-form>
                            </td>
                            <td>
                                <div v-for="obj in item.feed" class="text-left">
                                    <span class="text-left">{{obj.DrReferenceType}} {{obj.DrStartTime_convert}} - {{obj.DrEndTime_convert}}</span>
                                </div>
                                <el-button @click="toggleInput(item)" class="link-button float-left"
                                           v-if="item.hideToggle !== true">
                            ...
                            <td>
                                <label class="" v-if="item.poop!==null">
                                <span v-for="obj of item.poop">
                                    <span class="daily-activity_obj">{{obj}}</span>
                                </span>
                                </label>
                            </td>
                            <td>
                                <label class="">
                                <span v-for="obj of item.hygiene">
                                    <span class="daily-activity_obj">{{obj}}</span>
                                </span>
                                </label>
                            </td>
...
<script>
    ...
    export default {
        ...
        methods: {
            toggleInput(item) {
                item.selectionRest = true;
                item.hideToggle = true;
                this.$forceUpdate();
            },
            AddAction(e, item, action) {
                item.checked = true;
                if (action === "step1") {
                    item.selectionRest = false;
                    item.selectionCurrentTime = true;
                }else if (action === "step2") {
                    item.selectionCurrentTime = false;
                    item.selectionEndTime = true;
                } else if (action === "final") {
                    this.addDailyRoutine(item);
                }
            },
            addDailyRoutine(item) {
                    let btnAddObject = [];
                    btnAddObject.DrStartTime = this.txtTimeStart;
                    btnAddObject.DrEndTime = this.txtTimeStop;
                    btnAddObject.DrRemark = "";
                    btnAddObject.studentIDArray = JSON.stringify(item.Student_ID);
                    btnAddObject.DrReferenceType = "Rest|" + this.ddlRest;
            ...
            async LoadClass() {
                try {
                    ...
                    if (studentResponse) {
                        ...
                        tempStudent = studentResponse.Table;

                        // this.selectedClassStudent.forEach(m => {
                        //     const response = await DataSource.shared.getDailyRoutineByStudentID(m.Student_ID);
                        //     console.log(response)
                        // });

                        for (let obj of tempStudent) {

                            const response = await DataSource.shared.getDailyRoutineByStudentID(obj.Student_ID);

                            if (response.Table) {
                                let arrPromise = response.Table;

                                // Feed
                                let feed = [];
                                let tempFeed = arrPromise.filter(d => {
                                    return d.MainReferenceType === "Feed";
                                });
                                feed = tempFeed.map(m => m.DrReferenceType);

                                let replaceFeed = [];
                                feed.forEach(m => {
                                    replaceFeed.push(m.substring(5));
                                });
                                feed = replaceFeed;

                                // Rest
                                let rest = [];
                                let tempRest = arrPromise.filter(d => {
                                    return d.MainReferenceType === "Rest";
                                });
                                rest = tempRest;
                                // rest = tempRest.map(m => {
                                //     m.DrReferenceType = m.DrReferenceType + m.DrStartTime_convert + m.DrEndTime_convert;
                                // });
                                //
                                // let replaceRest = [];
                                // rest.forEach(m => {
                                ...
                                // Poop
                                let poop = [];
                                let tempPoop = arrPromise.filter(d => {
                                    return d.MainReferenceType === "Poop";
                                });
                                poop = tempPoop.map(m => m.DrReferenceType);

                                let replacePoop = [];
                                poop.forEach(m => {
                                    replacePoop.push(m.substring(5));
                                });
                                poop = replacePoop;

                                // Hygiene
                                let hygiene = [];
                                let tempHygiene = arrPromise.filter(d => {
                                    return d.MainReferenceType === "Hygiene";
                                });
                                hygiene = tempHygiene.map(m => m.DrReferenceType);

                                let replaceHygiene = [];
                                hygiene.forEach(m => {
                                    replaceHygiene.push(m.substring(8));
                                });
                                hygiene = replaceHygiene;
                        ...
                        console.log(tempStudent, "temp");
                        this.selectedClassStudent = tempStudent;
                        this.selectedClassStudent.forEach(m => {
                            m.checked = false;
                            m.selectionCurrentTime = false;
                            m.selectionRest = false;
                            m.hideToggle = false;
                            m.selectionEndTime = false;
                        });
                        console.log(this.selectedClassStudent, "selectedClassStudent");
