<template>
    <div class="mediaList" v-if="renderComponent">
        <sideBar/>

        <div class="content-body-container col-md-8">
            <!--            media header-->
            <div class="media-list-header-container">
                <span style="display: inline-block; vertical-align: middle;"><i class="fa fa-folder-open-o fa-2x sidebar-logo" aria-hidden="true"></i></span>

                <div style="display: inline-block; vertical-align: middle;">
                    <el-breadcrumb separator-class="el-icon-arrow-right">
                        <el-breadcrumb-item v-for="(obj_Folder,i) of arrobj_FolderPath"
                                            :key="obj_Folder.GalID"><span style="font-weight: bold; cursor: pointer;" @click="backSpecificDirectory(obj_Folder,i)">{{obj_Folder.GalFolder}}</span>
                        </el-breadcrumb-item>
                    </el-breadcrumb>
                </div>

                <!--                <label style="display: inline-block!important;">Folder name 02/02/2018</label>-->
                <a class="upload-item" style="color: #007bff; font-weight: unset; margin-left: 10px; cursor: pointer; vertical-align: middle;">Add Media</a>
                ...
                <el-popover style="float: right;"
                    ...
                    <!--                    <button class="btn btn-link">Download Folder</button>-->
                    <button @click="downloadItem" class="btn btn-link">Download Selected</button>
                    <el-button slot="reference"><i class="fa fa-ellipsis-v" aria-hidden="true"></i></el-button>
                </el-popover>

                <!--                <span style="float: right; padding: 8px 20px; vertical-align: middle; cursor: pointer;"><i class="fa fa-ellipsis-v" aria-hidden="true"></i></span>-->
                <button @click="showCreateModal" class="gallery-createFolder btn btn-primary"
                        title="New Folder" style="padding: 5px 10px; display: inline-block; float: right;"><i
                        class="fa fa-plus"></i>
            ...
            <!--            media folder list-->
            <div class="media-file-folder-list-container div_ItemContainer">
                <div class="">
                    <div class="single-folder-container" v-for="obj_Folder of arrobj_Folders" :key="obj_Folder.GalID"
                         :data-type="obj_Folder.GalType">
                        <button class="btn btn-media-folder" :id="obj_Folder.GalID" @click="changeDirectory(obj_Folder)">
                            <i class="material-icons">
                                perm_media
                            </i>
                            <span style="margin-left: 10px">{{obj_Folder.GalFolder}}</span>
                            <i class="fa fa-angle-right" aria-hidden="true" style="float:right; margin: 5px 0px;"></i>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <!--            media action bar-->
            <div class="" style="text-align: right; padding: 10px;" v-if="arrobj_Files.length > 0">
                <el-button-group>
            ...
            <div class="empty-list_image" v-if="arrobj_Files.length <= 0">
                <strong>{{empty_message}}</strong>
                <img src="../assets/notfound.png"/>
            </div>

            <!--            media list-->
            <div class="bg-white div_ItemContainer">

                <div class="single-media col-md-3" v-for="obj_File of arrobj_Files" :key="obj_File.GalID">
                    <!--                    <div class="media-file-list">-->
                    <!--                        <img class="single-media-img" v-if="isImage(obj_File)" :src="getLowSource(obj_File)" :alt="obj_File.GalFileName"-->
                    <!--                             draggable="false"/>-->
                    <!--                    </div>-->
                    <div class="media-file">
                        <label :for="obj_File.GalID" class="media-file-image-container">
                            <img class="media-file-image" v-if="isImage(obj_File)" :src="getLowSource(obj_File)" :alt="obj_File.GalFileName" draggable="false"/>
                        </label>

                        <input class="limited" type="checkbox" hidden="hidden" :id="obj_File.GalID" :value="obj_File"
                    ...
                    <!--                    <div class="media-file-more-action">-->
                    <!--                        <span style="margin-right: 10px;"><i class="fa fa-thumbs-o-up fa-2x sidebar-logo" aria-hidden="true"></i> 10 likes</span>-->
                    <!--                        <span><i class="fa fa-comments-o fa-2x sidebar-logo" aria-hidden="true"></i> 5 comments</span>-->
                    <!--                    </div>-->
                </div>
            ...
            <input type="file" id="fileUpload" multiple hidden="hidden"/>
    ...
    export default {
        ...
        methods: {
            ...
            initFolder(obj_Folder) {
                ...
                // if (this.arrobj_Folders.length <= 0) {
                //     this.arrobj_Folders = obj_Folder.GalFolder;
                // }
            },
        ...
        mounted() {
            ...
            $(document).ready(function(){
                ...
                $(".media-file-folder-list-container").appendTo(".sidebar-action-container .action-body");

                // $(window).scroll(function() {
                //     if($(window).scrollTop() === ($(document).height() - $(window).height())) {
                //         if (self.int_EndRowNo % 10 === 0) {
                //             let obj_CurrentFolder = self.arrobj_FolderPath[self.arrobj_FolderPath.length - 1];
                //             self.getFiles(obj_CurrentFolder).then((result) => {
                //                 self.arrobj_Files.push.apply(self.arrobj_Files, result);
                //             }).then(() => {
                //                 self.hideLoading();
                //             });
                //         }
            ...
            $(document).off("click", ".create-item").on("click", ".create-item", () => {
                self.showCreateModal();
            });
            $(document).off("click", ".rename-item").on("click", ".rename-item", () => {
                self.showRenameModal();
            });
            $(document).off("click", ".download-item").on("click", ".download-item", () => {
                self.downloadItem();
            });
            $(document).off("click", ".upload-item").on("click", ".upload-item", () => {
                $("#fileUpload").click();
            });
            $(document).off("click", ".view-item").on("click", ".view-item", () => {
                self.showViewModal();
            });
            $(document).off("click", ".move-item").on("click", ".move-item", () => {
                self.showMoveModal();
            });
            $(document).off("click", ".remove-item").on("click", ".remove-item", () => {
                self.showRemoveModal();
            });
            /*#endregion*/

            /*#region Custom Context Functions*/
            $(document).off("contextmenu", ".div_ItemContainer").on("contextmenu", ".div_ItemContainer", (e) => {
                self.showContextMenu(e);
            });

            $(document).off("contextmenu", "#div_ContextMenu").on("contextmenu", "#div_ContextMenu", (e) => {
                e.preventDefault();
            });
...
<style scoped>
    ...
    .media-list-header-container {
        text-align: left;
        margin-bottom: 20px;
    }

    .media-list-header-container > span {
        margin-right: 10px;
    }
    ...
    .media-file {
        ...
        border: 1px solid #d8dbde87;
        position: relative;
    }

    .media-file-image-container {
        min-height: inherit;
        border-radius: inherit;
        margin: 0px;
        max-height: 200px;
    ...
    .media-file input[type='checkbox'] + label {
        width: 20px;
        height: 20px;
        position: absolute;
        margin: auto;
    ...
    .media-file input[type='checkbox'] + label:before {
        content: '';
        width: 12px;
        height: 12px;
        top: 0;
    ...
    .media-file input[type='checkbox']:checked + label:before {
        bottom: 0;
    }
    ...
    .media-file-more-action {
        padding: 5px 0px;
    }

    .media-file-more-action > span > i {
    ...
    /*#end region Custom context menu*/
